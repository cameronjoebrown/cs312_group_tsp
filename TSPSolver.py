		...
		while not foundTour and time.time() - start_time < time_allowance:
			...
			for i in range(len(route)):
				if i == len(route) - 1:
					...
					next_city = i + 1
				distance = route[i].costTo(route[next_city])
				for city_not_in_route in cities_not_in_route:
					cost_to_insert = (route[i].costTo(city_not_in_route) + city_not_in_route.costTo(route[next_city])) - distance
					if cost_to_insert < nearest_distance:
						nearest_city = city_not_in_route
						nearest_city_index = cities_not_in_route.index(nearest_city)
						nearest_distance = cost_to_insert
						insert_index = i + 1
